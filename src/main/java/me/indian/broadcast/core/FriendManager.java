...
import me.indian.broadcast.core.models.friend.FriendModifyResponse;
import me.indian.broadcast.core.models.friend.FriendStatusResponse;
    ...
    public void initAutoFriend(FriendSyncConfig friendSyncConfig) {
        if (friendSyncConfig.isAutoFollow() || friendSyncConfig.isAutoUnfollow()) {
            sessionManager.scheduledThread().scheduleWithFixedDelay(() -> {
                // Auto Friend Checker
                try {
                    for (FollowerResponse.Person person : get(friendSyncConfig.isAutoFollow(), friendSyncConfig.isAutoUnfollow())) {
                    ...
                    try {
                        ...
                        if (response.statusCode() == 204) {
                            ...
                            if (modifyResponse.code() == 1028) {
                                ...
                                logger.warning("Removed " + entry.getValue() + " (" + entry.getKey() + ") as a friend due to account restrictions");
                            } else {
                                logger.warning("Failed to add " + entry.getValue() + " (" + entry.getKey() + ") as a friend: (" + response.statusCode() + ") " + response.body());
                            }
                        }
